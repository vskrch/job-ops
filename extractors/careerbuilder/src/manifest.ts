...
import { runCareerBuilder } from "./run";
...
function toProgress(event: {
  ...
  if (event.type === "term_start") {
    return {
      ...
      detail: `CareerBuilder: term ${event.termIndex}/${event.termTotal} (${event.searchTerm})`,
    };
  }
  return {
    phase: "list",
    ...
    detail: `CareerBuilder: completed ${event.termIndex}/${event.termTotal} (${event.searchTerm}) with ${event.jobsFoundTerm ?? 0} jobs`,
  };
}
...
export const manifest: ExtractorManifest = {
  ...
  async run(context) {
    ...
    const result = await runCareerBuilder({
      selectedCountry: context.selectedCountry,
      searchTerms: context.searchTerms,
      maxJobsPerTerm,
      fetchImpl: createCrawledFetch({ source: "careerbuilder" }),
      onProgress: (event) => {
