...
function toProgress(event: {
  ...
  if (event.type === "term_start") {
    return {
      ...
      detail: `HN Who's Hiring: term ${event.termIndex}/${event.termTotal} (${event.searchTerm})`,
    };
  }
  return {
    phase: "list",
    ...
    detail: `HN Who's Hiring: completed ${event.termIndex}/${event.termTotal} (${event.searchTerm}) with ${event.jobsFoundTerm ?? 0} jobs`,
  };
}
...
export const manifest: ExtractorManifest = {
  ...
  async run(context) {
    ...
    const result = await runHnHiring({
      ...
      maxJobsPerTerm,
      fetchImpl: createCrawledFetch({ source: "hnhiring" }),
      onProgress: (event) => {
