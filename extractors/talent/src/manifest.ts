...
function toProgress(event: {
  ...
  if (event.type === "term_start") {
    return {
      ...
      detail: `Talent.com: term ${event.termIndex}/${event.termTotal} (${event.searchTerm})`,
    };
  }
  return {
    phase: "list",
    ...
    detail: `Talent.com: completed ${event.termIndex}/${event.termTotal} (${event.searchTerm}) with ${event.jobsFoundTerm ?? 0} jobs`,
  };
}
...
export const manifest: ExtractorManifest = {
  ...
  async run(context) {
    ...
    const result = await runTalent({
      searchTerms: context.searchTerms,
      selectedCountry: context.selectedCountry,
      maxJobsPerTerm,
      fetchImpl: createCrawledFetch({ source: "talent" }),
      onProgress: (event) => {
