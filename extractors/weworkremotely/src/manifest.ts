...
function toProgress(event: {
  ...
  if (event.type === "term_start") {
    return {
      ...
      detail: `We Work Remotely: term ${event.termIndex}/${event.termTotal} (${event.searchTerm})`,
    };
  }
  return {
    phase: "list",
    ...
    detail: `We Work Remotely: completed ${event.termIndex}/${event.termTotal} (${event.searchTerm}) with ${event.jobsFoundTerm ?? 0} jobs`,
  };
}
...
export const manifest: ExtractorManifest = {
  ...
  async run(context) {
    ...
    const result = await runWwr({
      ...
      maxJobsPerTerm,
      fetchImpl: createCrawledFetch({ source: "weworkremotely" }),
      onProgress: (event) => {
